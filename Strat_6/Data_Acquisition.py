...
from module_1.techinical_analysis import * 
...
# BAR STATS 
# =============================================================================
green_stats = pf.dist_stats(df[df['labels'] == 1], 'gap')
red_stats = pf.dist_stats(df[df['labels'] == 0], 'gap')
...
MODEL_PATH = Path(file_dir + f"/data/{ticker}")
MODEL_PATH.mkdir(parents = True, exist_ok = True)
try:
    df1.to_parquet(f"{strat}/data/{ticker}/DF_{end_date.strftime('%Y_%m_%d')}.parquet")
